...
const Verifedusers = () => {
    ...
    useEffect(() => {
        if (userdata) {
            const verifiedUsers = userdata.data.filter(item => item.emailVerified === true);
    ...
    useEffect(() => {
        if (searchTerm.trim() === '') {
            setFilteredUsers(userdata?.data?.filter(item => item.emailVerified === true) || []);
        } else {
            const filtered = userdata?.data?.filter(item =>
                item.username.toLowerCase().includes(searchTerm.toLowerCase()) ||
                item.email.toLowerCase().includes(searchTerm.toLowerCase())
            );
            setFilteredUsers(filtered.filter(item => item.emailVerified === true));
        }
    }, [searchTerm, userdata]);
